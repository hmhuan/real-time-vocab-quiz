...
1. **System Design Document**:
   ...
   ![alt text](public/diagram_5.png)
   - **Component Description**: Describe each component's role in the system.
     - client: app 
     - cdn: stores user's profile image, ... 
     - gateway: gateway of whole app
     - load balancer: load balancer of quiz service
     - servers: multiple servers of quiz service
     - databases: plan to use postgresql
     - redis: caching user data, quiz data, quiz session data, leaderboard, ...
   - **Data Flow**: Explain how data flows through the system from when a user joins a quiz to when the leaderboard is updated.
      - This maybe internal our app flow
         - Admin -> create question
         - Admin -> create quiz (questions randomly chosen)

      - Admin -> host quiz (this step maybe handle automatically) -> generate unique code 
      - Users join  by code -> open websocket -> wait lobby until admin (or handling automatically by job) start
      - When users answer question -> update user score and leaderboard of quiz -> reflect data by websocket communication
   ...
   - **Technologies and Tools**: List and justify the technologies and tools chosen for each component.
     ...
     - Postgresql
     - Redis
     - React (for web app using react, maybe native app we using another technology)
